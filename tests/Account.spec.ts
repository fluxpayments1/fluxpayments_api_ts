...
import { AccountQuery } from "fluxpayments/flux_types";
import { Address } from "fluxpayments/flux_types";
import { AddressQuery } from "fluxpayments/flux_types";
import { Account, AccountUserType, IAccount } from "fluxpayments/flux_types";
...
describe("Account specific tests", function () {
    ...
    it("should have created 2 addresses and added them to the first account", async () => {
        adds = (await acc.getAddresses()).sort((a, b) => a.id - b.id);
    ...
    it("should have created 2 addresses and added them to the second account", async () => {
        adds2 = (await acc2.getAddresses()).sort((a, b) => a.id - b.id);
    ...
    it("should not allow you to add the same address twice", async () => {
        ...
        assert.lengthOf(await acc.getAddresses(), 2)
    ...
    it("should manage addresses correctly and refresh the first acc correctly", async () => {
        ...
        let newAdds = await acc.getAddresses();
        assert.lengthOf(newAdds, 0);

        let newAdds2 = await acc2.getAddresses();
        assert.lengthOf(newAdds2, 2);

        await acc2.removeAddress(newAdds2);
        newAdds2 = await acc2.getAddresses();
