...
import { Product } from "fluxpayments/flux_types";
import { ProductQuery } from "fluxpayments/flux_types";
...
describe("Product specific tests", function () {
    let fluxObj: Flux<any>, prod: Product, prod2: Product;
    ...
    it("should merge changes correctly, refresh objects correctly, and make correct use of load lazy", async () => {
        let product = await Product.createInstanceLazy({
            uniqueId: "FIRSTPRODUCT",
        })
        ...
        product = await Product.createInstanceLazy({
            uniqueId: "FIRSTPRODUCT",
        })
    ...
    after(async () => {
        ...
        let prodSearch = await Product.queryObjects(ProductQuery.createQuery({}));
