import { FluxIdentifier, Address } from "fluxpayments/flux_types";
...
describe("Flux Address CRUD Tests", function () {
    ...
    before(async () => {
        try {
            ...
            addressIdentifier = [(await Address.createInstanceSafe(await Address.createInstanceLazy(address))).getId()];
            newAddress = await Address.updateObjects(await Address.createInstanceLazy({...address, addressState: "Arkansas"}))
        ...
        try {
            addressSearch = await Address.queryObjects(new AddressQuery({}));
            deletedAddress = await Address.deleteObjects(newAddress[0].getId())
            searchedAddressesEmpty = await Address.queryObjects(new AddressQuery({}));
        } catch (e: any) {
    ...
    it ("should return a fluxIdentifier when the getId method is called", async () => {
        let f: FluxIdentifier = (await Address.createInstanceLazy(address)).getId()
...
describe("Pagination and additional functionalities tests", function () {
    ...
    before(async () => {
        ...
        while (addresses.length < 53) {
            try {
                let productIds = await Address.createObjects(createRandomAddress());
                addresses = addresses.concat(productIds)
            } catch (e: any) {
            }
        }
        try {
            await Address.queryObjects(new AddressQuery({
                pagination: {
                    pageNumber: 0,
                    itemsPerPage: 50
                }
        ...
        searchedAddressesPage1 = await Address.queryObjects(new AddressQuery({
            pagination: {
                pageNumber: 1,
                itemsPerPage: 50
            }
        ...
        searchedAddressesPage2 = await Address.queryObjects(new AddressQuery({
            pagination: {
                pageNumber: 2,
                itemsPerPage: 50
            }
        ...
        searchedAddressesPage3 = await Address.queryObjects(new AddressQuery({
            pagination: {
                pageNumber: 3,
                itemsPerPage: 50
            }
        ...
        orderedAddressesByName = (await Address.queryObjects(new AddressQuery({
            pagination: {
                pageNumber: 1,
                itemsPerPage: 50
            },
        ...
        await Address.deleteObjects(addresses);
