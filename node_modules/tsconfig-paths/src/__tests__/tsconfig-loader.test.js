...
describe("loadConfig", () => {
    ...
    it("should load a config with array extends and overwrite all options", () => {
        ...
        const baseConfig1Path = (0, path_1.join)("/root", "base-config-1.json");
        const baseConfig2 = { compilerOptions: { baseUrl: "." } };
        const baseConfig2Path = (0, path_1.join)("/root", "dir1", "base-config-2.json");
        const baseConfig3 = {
            compilerOptions: { baseUrl: ".", paths: { foo: ["bar2"] } },
        };
        const baseConfig3Path = (0, path_1.join)("/root", "dir1", "dir2", "base-config-3.json");
        const actualConfig = {
            extends: [
                "./base-config-1.json",
        ...
        const actualConfigPath = (0, path_1.join)("/root", "tsconfig.json");
        const res = (0, tsconfig_loader_1.loadTsconfig)((0, path_1.join)("/root", "tsconfig.json"), (path) => [
            baseConfig1Path,
            baseConfig2Path,
            baseConfig3Path,
            actualConfigPath,
    ...
    it("should load a config with array extends without .json extension", () => {
        ...
        const baseConfigPath = (0, path_1.join)("/root", "base-config-1.json");
        const actualConfig = { extends: ["./base-config-1"] };
        const actualConfigPath = (0, path_1.join)("/root", "tsconfig.json");
        const res = (0, tsconfig_loader_1.loadTsconfig)((0, path_1.join)("/root", "tsconfig.json"), (path) => [baseConfigPath, actualConfigPath].indexOf(path) >= 0, (path) => {
            if (path === baseConfigPath) {
                return JSON.stringify(baseConfig);
            }
            if (path === actualConfigPath) {
